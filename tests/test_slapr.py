...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            [Message(text="Need :eyes: <https://github.com/example/repo/pull/42>", timestamp="yyyy-mm-dd")],
            [Review(state="approved", user=_user("alice")), Review(state="commented", user=_user("alice"))],
            [],
            ["test_approved"],
            id="commented-after-approval-same-reviewer",
        ),
        pytest.param(
            [Message(text="Need :eyes: <https://github.com/example/repo/pull/42>", timestamp="yyyy-mm-dd")],
            [
                Review(state="approved", user=_user("alice")),
                Review(state="commented", user=_user("codex-bot")),
                Review(state="commented", user=_user("alice")),
            ],
            [],
            ["test_approved"],
